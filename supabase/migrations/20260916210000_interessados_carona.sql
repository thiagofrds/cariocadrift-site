...
-- Aplicar somente com autorização expressa. Reversão: drop table public.interessados_carona cascade; drop table public.tentativas_carona; drop function public.limita_envios_carona();
...
-- O visitante só pode informar as colunas do formulário: id e criado_em são do banco (privilégio por coluna).
...
-- tentativas de envio (inclusive duplicatas ignoradas): base dos limites. Sem nenhum acesso pela API.
create table public.tentativas_carona (
  telefone  text not null,
  criado_em timestamptz not null default now()
);
create index tentativas_carona_criado_em on public.tentativas_carona (criado_em);
create index tentativas_carona_telefone on public.tentativas_carona (telefone, criado_em);
alter table public.tentativas_carona enable row level security;
revoke all on public.tentativas_carona from public, anon, authenticated;
...
-- · os limites contam TENTATIVAS (tabela tentativas_carona), não registros: duplicata conta igual a envio novo,
--   então um telefone cadastrado e um telefone novo recebem exatamente as mesmas respostas em qualquer situação;
-- · criado_em é sempre definido pelo banco, mesmo que o cliente tente informar;
-- no máximo 60 tentativas por 10 minutos no total, e no máximo 3 por telefone por hora.
...
begin
  ...
  -- limites por tentativa, verificados ANTES de olhar duplicidade (mesma resposta para telefone novo ou já cadastrado)
  if (select count(*) from public.tentativas_carona where criado_em > now() - interval '10 minutes') >= 60 then
    raise exception 'limite de envios atingido, tente mais tarde' using errcode = 'P0001';
  end if;
  if (select count(*) from public.tentativas_carona where telefone = new.telefone and criado_em > now() - interval '1 hour') >= 3 then
    raise exception 'limite de envios atingido para este telefone' using errcode = 'P0001';
  end if;
  insert into public.tentativas_carona (telefone) values (new.telefone);
  delete from public.tentativas_carona where criado_em < now() - interval '2 hours';   -- higiene, nada sensível fica além do necessário
  ...
  new.criado_em := now();   -- nunca vem do cliente
  return new;
end $$;
...
-- 1b. anon enviando id, ou criado_em (antigo ou futuro) → 42501 (sem privilégio na coluna); nenhuma linha gravada.
