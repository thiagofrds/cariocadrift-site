...
(async () => {
  ...
  const abre = async (vp, mobile) => { const ctx = await browser.newContext({ viewport: vp, deviceScaleFactor: 2, isMobile: mobile, hasTouch: mobile }); const p = await ctx.newPage(); const erros = []; p.on('console', m => { if (m.type() === 'error') erros.push(m.text()); }); await p.goto(url, { waitUntil: 'networkidle' }); await p.waitForTimeout(1200); await p.evaluate(() => document.fonts.ready); await p.evaluate(() => Promise.all([...document.querySelectorAll('video')].map(v => new Promise(r => { if (!v.videoWidth && !v.readyState) return r(); v.pause(); v.currentTime = 2; v.onseeked = () => r(); setTimeout(r, 2500); })))); await p.waitForTimeout(400); const l = await p.evaluate(() => ({ s: document.documentElement.scrollWidth, i: innerWidth })); if (l.s > l.i) problemas.push(`${vp.width}: rolagem horizontal ${l.s}`); if (erros.length) problemas.push(`${vp.width}: ${erros.join(' | ')}`); return { ctx, p }; };
